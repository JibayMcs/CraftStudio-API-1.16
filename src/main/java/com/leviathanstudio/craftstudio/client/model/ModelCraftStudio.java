 ...
 */
@OnlyIn(Dist.CLIENT)
public class ModelCraftStudio extends Model
{
    ...
    private CSModelRenderer generateCSModelRend(CSReadedModelBlock rBlock) {
        ...
        if (rBlock.getVertex() != null) {
            CSModelRenderer.PositionTextureVertex vertices[] = new CSModelRenderer.PositionTextureVertex[][8];
            for (int i = 0; i < 8; i++)
                vertices[i] = new CSModelRenderer.PositionTextureVertex(rBlock.getVertex()[i][0], rBlock.getVertex()[i][1], rBlock.getVertex()[i][2], 0.0F, 0.0F);
            modelRend.addBox(vertices, CSModelRenderer.ModelBox.getTextureUVsForRect(rBlock.getTexOffset()[0], rBlock.getTexOffset()[1], rBlock.getSize().x,
